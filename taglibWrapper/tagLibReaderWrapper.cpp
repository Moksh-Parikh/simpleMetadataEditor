...
void writeTagSettings(const TagLib::Tag *tag, TagSettings* tag_settings) {
        ...
        snprintf(tag_settings->date, sizeof(tag_settings->date),
                 "%d", (int)tag->year());
        tag_settings->date[sizeof(tag_settings->date) - 1] = '\0';
}

void writeReplayGainInfo(TagLib::MPEG::File* mp3File, TagSettings* tag_settings) {
    if (mp3File) {
                ...
                double *duration, char** lyrics)
{
        ...
        writeTagSettings(tag, tag_settings);

        TagLib::MPEG::File *mpegFile;
        if (lyrics != nullptr && *lyrics == nullptr) {
            if ((mpegFile = dynamic_cast<TagLib::MPEG::File *>(f.file()) ) ) {
        ...
        writeReplayGainInfo(mpegFile, tag_settings);
